...
依[正式帳戶資料接入與對帳計畫](account_data_integration_plan.md)，AC1～AC4 fixture 版已完成：具備嚴格契約、原始檔封存、確定性 NAV／現金對帳與決策 adapter。這是選配的外部結算檔對帳支線。每日主線的 VA1～VA3 fixture 工具鏈也已完成，可建立唯一 10 億 TWD 空倉帳戶、續接帳本、建立決策前快照並封存模擬成交；VA4 尚未接入報告工作流，VA5 真實資料與前向驗收未完成。交易狀態、基準及規則仍須獨立驗收。
