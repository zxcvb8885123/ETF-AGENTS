...
狀態：架構設計完成，資料蒐集第一版已實作。依據 [三模組開發規劃](agent_plan.md)。

## 1. 整體設計

採用一個 Python 應用程式，由每日流程控制器依序呼叫三個模組。第一版使用檔案保存資料快照與執行結果，無須先部署多個服務。LLM 透過可替換的介面接入，模型選擇與資料供應商待實作時確定。
...
交易池、帳戶、必要價格或 ETF 基準缺失時停止該次流程；個別新聞缺失則標記缺失，不自動解讀為利多或利空。官方名單是否已發布必須另行確認，目前本地只有空白模板。
...
目前已有基本資料模型、部分規則檢查、SQLite schema、官方交易池匯入、TWSE 最新交易日行情收集、原始回應與執行紀錄保存。實際資料管線已用 TWSE 2026-09-11 資料驗證。現有 guard 只接受單一 benchmark，尚未做前十大裁切、全部 ETF 比對、完整輸入驗證、成交模擬或完整 Agent 稽核持久化；資料抓取成功不代表即可正式送件。

1. **資料契約與離線流程**：行情收集與 SQLite 已完成第一版；下一步建立研究快照契約、完整資料品質檢查，並用明確標記的測試資料跑通三模組。
2. **量化策略與帳本**：候選排序、配置、整張交易、費稅與公司行動，核對預估現金和 NAV。
3. **完整風控**：補齊官方計算口徑、多 ETF 比對、超限追蹤、情境測試與拒絕分支。
4. **真實資料與 LLM**：接入選定來源，保存證據與模型輸出，測試過期資料及模型失敗備援。
5. **歷史驗證及送件格式**：用時間一致的歷史資料比較基準策略與 LLM 版本，完成官方格式、回執與重複送件防護。
