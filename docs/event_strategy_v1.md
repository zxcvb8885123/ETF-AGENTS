...
LLM 負責事件理解、補查選擇與理由。營收增減、財報期間比較、動能、價格確認及後續權重與費稅由程式計算。第一版事件研究不負責社群情緒或分析師目標價；這兩類留給未來獨立的「市場情緒與分析師研究 Agent」，通過來源與時間驗證後才可作次級研究輸入。LLM 自報信心不能當作上漲機率，低信心也不等同中性。LLM 不直接輸出可執行的 `buy`／`sell`、最終持股權重或主觀 1～10 信心分數。
