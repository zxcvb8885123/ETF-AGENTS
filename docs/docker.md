...
./start.sh daily     # 一鍵驗證來源、更新行情／事件、建立 Snapshot 並交付報告
./start.sh report    # 使用既有 Snapshot 執行或續跑報告工作流
```

`daily` 會先驗證官方來源與交易池，再更新上市／上櫃行情、歷史行情及月營收／重大訊息，最後以 `DAILY_CUTOFF`（未設定時使用 Asia/Taipei 現在時間）建立 Snapshot 並執行報告工作流。結果固定在 `artifacts/reports/latest.md`；缺少事件研究 Agent 輸出時會交付 waiting／blocked 狀態。`report` 只使用既有 Snapshot 執行或續跑工作流。兩者都不會自動產生或送出訂單。
