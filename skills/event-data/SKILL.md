---
name: event-data
description: 探測台灣市場官方資料來源、驗證競賽交易池、收集並版本化事件資料，以及建立時間點研究快照。用於檢查、更新或準備 Data Agent 輸入；不得用於事件評分或交易決策。
metadata:
...
5. 重播歷史決策時，執行 `snapshot` 並明確提供含時區的 `--decision-cutoff`。只有確定要使用目前 UTC 時間時才能省略。
6. 回報所有 `QUALITY` 旗標。不可用的快照不得交給策略。只有明確要求診斷時才能使用略過驗證的參數，而且該輸出不能作為正式研究輸入。
7. 保留 `latest_prices` 與 `documents` 中的每個 `source_evidence_id`。`source_evidence` 是交接給下游 D-Plan 的稽核橋梁；不得在 Data Agent 內配置送件專用的 `S1`／`O1` ID。
